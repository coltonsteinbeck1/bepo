...
/**
 * Check if a user has permission to manage channels based on guild_roles
 * @param {string} guildId - Discord guild ID
 * @param {Array<string>} userRoleNames - Array of role names the user has
 * @param {Array<string>} allowedRoleNames - Array of role names that grant permission (e.g., ['Degenerate', 'Code Monkey'])
 * @returns {boolean} Whether user has permission
 */
async function hasChannelManagementPermission(guildId, userRoleNames, allowedRoleNames = ['Degenerate', 'Code Monkey']) {
  try {
    // Query guild_roles table for roles that match the allowed role names
    const { data, error } = await supabase
      .from('guild_roles')
      .select('role_name')
      .eq('guild_id', guildId)
      .in('role_name', allowedRoleNames);
    ...
    if (!data || data.length === 0) {
      // No matching roles configured in the database for this guild
      return false;
    }

    // Get the allowed role names from the database
    const dbAllowedRoles = data.map(row => row.role_name.toLowerCase());

    // Check if user has any of the allowed roles
    const userRolesLower = userRoleNames.map(name => name.toLowerCase());
    const hasPermission = userRolesLower.some(userRole =>
      dbAllowedRoles.includes(userRole)
    );

    return hasPermission;
  } catch (error) {
    console.error('Error in hasChannelManagementPermission:', error);
    return false;
  }
}

/**
 * Get all allowed roles for channel management in a guild
 * @param {string} guildId - Discord guild ID
 * @returns {Array} List of role names allowed to manage channels
 */
async function getChannelManagementRoles(guildId) {
  try {
    const { data, error } = await supabase
      .from('guild_roles')
    ...
    if (error) {
      console.error('Error fetching channel management roles:', error);
      return [];
    }

    return data || [];
  } catch (error) {
    console.error('Error in getChannelManagementRoles:', error);
    return [];
  }
}
...
export {
  ...
  getVideoStats,
  cleanupOldVideos,
  // Channel management permission functions
  hasChannelManagementPermission,
  getChannelManagementRoles
}
